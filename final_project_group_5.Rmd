...
where $\hat{cor}(\epsilon, \text{time})$ is sample correlation between the error vector $\epsilon$ and the year week in the data. The larger this quantity is for the model, the less well it fits the data (since, if a regression model actually fits, the errors should ideally be uncorrelated with the predictor).
...
```{r}
T.sim = abs(cor(t(error.sim), fdp_train$FanDuelPts))
head(T.sim)
```

*The simulated values of $T(y^{rep}, X, \theta )$*
```{r}
T.rep.sim = abs(cor(t(error.rep), fdp_train$FanDuelPts))
head(T.rep.sim)
```
...
The p.value is `r p.value`.  This indicates the error actually has correlation with the response.
...
While the model has some issue with error correlation, it has decent prediction ability.  It should be noted that we only picked two positions to predict due to computation resource constraint.  In the pursue of creating this model, we have multiple route:
