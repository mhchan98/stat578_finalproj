...
Data cleaning is performed using R routines. Some data cleaning tasks are needed to calculate Player rank.  The code used to get the data from RotoGuru can be found in the Appendices - Data Section.
...
Due to significant roster change usually happens in the off-season, we believe it is the best to not use data across seasons.  We use 2016 data, `fdp_train`,  as it is the most recent year with a full season. We set aside the last week for verifying prediction accuracy of the model, `fdp_test`.
...
This session shows some sample X used in the computation.  `X.defense` captures the interaction terms between the defensive power (higher `AvgOppPAP7Wks` means the player is facing a weaker team, since they allows players score more point on them) and position.
...
The code used to set up the data used in the model computation is listed in *Appendices - Computation* section.
...
The trace plots of all parameters show good distribution convergence.  Please see *Appendices - Converage diagnostics* for all trace plots and the Gelman Statistics summary, and MCMC Summary
...
The effective sample sizes of all parameters are greater than 400. See appendics for full output.

\newpage
#Model Assessment

All code in this sections are listed in appendices
...
The p.value, `mean(T.rep.sim >= T.sim)` , is `r p.value`, > 0.05, which does not indicate any evidence of problem.
...
A measure of the effectiveness of this model is to predict individual player performance.  Consider an example data point, `fdp_test[1,]`. The real FanDuelPts is `r fdp_test$FanDuelPts[1]`
